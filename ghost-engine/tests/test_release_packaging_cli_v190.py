...
def test_v190_package_and_runtime_versions():
    assert _pyproject()["project"]["version"] == "1.11.0"
...
def test_v190_snapshot_schema_intentionally_remains_v1():
    ...
    assert snapshot["ghost_version"] == "1.11.0"
    assert snapshot["schema_version"] == "1.0"
    assert snapshot["engine"]["ghost_version"] == "1.11.0"
...
def test_v190_v180_producer_snapshot_remains_restoreable():
    ...
    assert current["ghost_version"] == "1.11.0"
    assert current["schema_version"] == "1.0"
    assert current["engine"]["ghost_version"] == "1.11.0"
...
def test_v190_readme_lists_version_validation_and_new_cli_commands():
    ...
    assert "Version:    1.11.0" in text
    assert "Validation: 2,745 passed, 1 skipped" in text
